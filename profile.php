...
<head>
    ...
    <style>
        ...
        * { font-family:'DM Sans',sans-serif; box-sizing:border-box; }
        ...
        .profile-header { background:linear-gradient(135deg,var(--deep),var(--green)); color:#fff; padding:36px; text-align:center; }
        ...
        .avatar-ring { width:100px; height:100px; border-radius:50%; background:rgba(255,255,255,.15); border:3px solid rgba(255,255,255,.5); margin:0 auto 14px; overflow:hidden; display:flex; align-items:center; justify-content:center; font-size:2.4rem; font-weight:700; color:#fff; cursor:pointer; position:relative; }
        .avatar-ring img { width:100%; height:100%; object-fit:cover; }
        .avatar-overlay { position:absolute; inset:0; background:rgba(0,0,0,.42); display:flex; align-items:center; justify-content:center; opacity:0; transition:.2s; border-radius:50%; }
        .avatar-ring:hover .avatar-overlay { opacity:1; }
        .badge-role { display:inline-block; padding:4px 14px; border-radius:20px; font-size:.72rem; font-weight:700; letter-spacing:1.5px; text-transform:uppercase; margin-top:6px; }
        .badge-user  { background:rgba(255,255,255,.2); color:#fff; }
        .badge-admin { background:#fee2e2; color:#b91c1c; }
        .form-control, .form-select { background:var(--sage); border:2px solid transparent; border-radius:12px; padding:.75rem 1rem; color:var(--deep); transition:.2s; }
        .form-control:focus, .form-select:focus { border-color:var(--green); background:#fff; box-shadow:none; color:var(--deep); }
        .btn-green { background:var(--green); color:#fff; border:none; border-radius:50px; padding:.65rem 1.8rem; font-weight:600; transition:.2s; text-decoration:none; display:inline-block; }
        .btn-green:hover { background:var(--deep); color:#fff; }
        .cart-item { display:flex; align-items:center; gap:12px; padding:12px 14px; background:var(--cream); border-radius:12px; margin-bottom:8px; transition:.15s; }
        ...
        .qty-stepper button { width:30px; height:30px; border:none; background:var(--sage); color:var(--green); font-weight:700; font-size:1rem; cursor:pointer; transition:.15s; line-height:1; }
        ...
        .st-completed, .st-delivered { background:#dcfce7; color:#16a34a; }
        ...
        .p-toast { position:fixed; bottom:28px; right:28px; background:var(--deep); color:#fff; padding:12px 22px; border-radius:50px; font-size:.85rem; font-weight:600; opacity:0; transform:translateY(12px); transition:.3s; pointer-events:none; z-index:9999; }
                ...
                <div class="order-box">
                    <div class="d-flex justify-content-between align-items-start mb-3 flex-wrap gap-2">
                        <div>
                            <div class="fw-bold" style="color:var(--deep);font-size:.9rem;">
                                ...
                                #<?= htmlspecialchars($oOrderId) ?>
                            </div>
                            <div class="text-muted" style="font-size:.74rem;margin-top:3px;">
                                <i class="fas fa-clock me-1"></i>
                                <?= $oDate ? date('F d, Y — h:i A', strtotime($oDate)) : '' ?>
                            </div>
                        </div>
                        <div class="d-flex align-items-center gap-2 flex-wrap">
                            <span class="order-status <?= $stCls ?>"><?= htmlspecialchars($oStatus) ?></span>
                            <a href="order.php?order_id=<?= urlencode($oOrderId) ?>"
                               class="btn btn-sm btn-outline-success rounded-pill" style="font-size:.72rem;padding:3px 12px;">
                    ...
                    <div style="background:var(--cream);border-radius:10px;padding:10px 14px;margin-bottom:12px;">
                        <?php foreach ($oItems as $oi): ?>
                            <?php
                            // FIX: order_items columns are product_name, qty, price
                            $oiName  = $oi->product_name ?? '?';
                            $oiQty   = (int)($oi->qty   ?? 1);
                            $oiPrice = (float)($oi->price ?? 0);
                            $oiLine  = $oiPrice * $oiQty;
                            $invId   = (int)($oi->inv_id ?? 0);
                            $curStock = $stockMap[$invId] ?? null;
                            ?>
                            <div class="d-flex align-items-center gap-2 py-2"
                                ...
                                <div style="flex:1;min-width:0;">
                                    ...
                                    <div style="margin-top:3px;">
                                        ...
                                        <?php else: ?>
                                            <span class="stock-chip sc-ok">In Stock</span>
                                        <?php endif; ?>
                                    </div>
                                    <?php endif; ?>
                                </div>
                        ...
                        <div style="margin-top:10px;">
                            ...
                            <div class="d-flex justify-content-between fw-bold"
                                 style="font-size:.9rem;color:var(--green);border-top:2px solid var(--sage);padding-top:8px;margin-top:6px;">
                    ...
                    <div class="d-flex flex-wrap gap-3" style="font-size:.76rem;color:#888;">
                        <?php if (!empty($oShipAddr)): ?>
                        <span><i class="fas fa-map-marker-alt me-1" style="color:var(--terra);"></i><?= htmlspecialchars($oShipAddr) ?></span>
                        <?php endif; ?>
                        <?php if (!empty($oPayMethod)): ?>
                        <span><i class="fas fa-credit-card me-1" style="color:var(--mid);"></i><?= htmlspecialchars($oPayMethod) ?></span>
                        <?php endif; ?>
                    </div>
                </div>
            <?php endforeach; ?>
