...
.order-card{background:#fff;border-radius:20px;box-shadow:0 4px 20px rgba(0,0,0,.07);margin-bottom:24px;overflow:hidden;}
.order-card-header{padding:18px 24px;border-bottom:2px solid var(--sage);display:flex;align-items:center;flex-wrap:wrap;gap:12px;}
.order-card-body{padding:22px 24px;}
.status-pill{display:inline-flex;align-items:center;gap:6px;padding:5px 14px;border-radius:50px;font-size:.72rem;font-weight:700;letter-spacing:.5px;text-transform:uppercase;}
...
.item-row{display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid #f0f0eb;}
...
.totals-mini{background:var(--cream);border-radius:12px;padding:14px 18px;margin-top:14px;}
...
.delivery-chip{display:inline-flex;align-items:center;gap:6px;background:var(--cream);border-radius:50px;padding:5px 14px;font-size:.78rem;color:#666;}
...
<nav class="navbar navbar-light px-4 py-2 fixed-top">
  ...
  <div class="ms-auto d-flex gap-2 align-items-center">
    ...
    <a href="orders.php" class="btn btn-sm btn-light rounded-pill">All Orders</a>
...
<div class="container py-4" style="max-width:900px;">
  ...
  <div class="empty-state">
    ...
    <a href="orders.php" class="btn btn-sm btn-outline-success rounded-pill mt-2 px-4">View All Orders</a>
  </div>
  <?php else: ?>
  ...
  <?php
    ...
    // FIX: flat columns, not nested object
    $oItems   = $o->items ?? [];
    $subtotal = (float)($o->subtotal ?? 0);
    $shipping = (float)($o->shipping ?? 150);
    $total    = (float)($o->total ?? ($subtotal + $shipping));
    $oDate    = $o->date ?? '';   // FIX: column is 'date'
    $orderId  = $o->order_id ?? '—';
    $payMethod= $o->pay_method ?? '';

    // FIX: flat columns for shipping info
  ...
  <div class="order-card" data-order-id="<?= htmlspecialchars($orderId) ?>">
    <div class="order-card-header">
      <div>
        <div style="font-weight:700;color:var(--deep);font-size:.9rem;"><i class="fas fa-tag me-1" style="color:var(--mid);"></i>#<?= htmlspecialchars($orderId) ?></div>
        <?php if ($oDate): ?>
        <div style="font-size:.74rem;color:#999;margin-top:2px;"><i class="fas fa-clock me-1"></i><?= date('F d, Y — h:i A', strtotime($oDate)) ?></div>
        <?php endif; ?>
      </div>
      <span class="status-pill <?= $stClass ?> dyn-status-badge"><i class="fas fa-circle" style="font-size:.4rem;"></i><?= htmlspecialchars($oStatus) ?></span>
      <?php if ($payMethod): ?>
      <span class="delivery-chip ms-auto"><i class="fas fa-credit-card" style="color:var(--mid);"></i><?= htmlspecialchars($payMethod) ?></span>
      <?php endif; ?>
      <a href="orders.php" class="btn btn-sm btn-outline-secondary rounded-pill" style="font-size:.75rem;">← All Orders</a>
    </div>
    ...
    <div class="order-card-body">
      ...
      <div class="dyn-status-msg" style="background:var(--cream);border-radius:12px;padding:10px 16px;margin-bottom:18px;font-size:.84rem;color:#555;">
        <?php $statusMsgs = [
      ...
      <div style="border:2px solid var(--sage);border-radius:14px;padding:8px 16px;margin-bottom:16px;">
        <?php foreach ($oItems as $oi):
          // FIX: order_items column is product_name
        ...
        ?>
        <div class="item-row">
          <div style="flex:1;min-width:0;"><div class="item-name"><?= htmlspecialchars($oiName) ?></div><div class="item-meta">₱<?= number_format($oiPrice, 2) ?> × <?= $oiQty ?></div></div>
      ...
      <?php
      $deliveryLine = implode(', ', array_filter([$fullName, $address, $city, $province]));
      if ($deliveryLine): ?>
      <div class="mt-3 d-flex flex-wrap gap-2">
        <span class="delivery-chip"><i class="fas fa-map-marker-alt" style="color:var(--terra);"></i><?= htmlspecialchars($deliveryLine) ?></span>
        <?php if ($phone): ?><span class="delivery-chip"><i class="fas fa-phone" style="color:var(--mid);"></i><?= htmlspecialchars($phone) ?></span><?php endif; ?>
        <?php if ($zip): ?><span class="delivery-chip"><i class="fas fa-envelope" style="color:var(--mid);"></i><?= htmlspecialchars($zip) ?></span><?php endif; ?>
        <?php if ($notes): ?><span class="delivery-chip"><i class="fas fa-sticky-note" style="color:var(--terra);"></i><?= htmlspecialchars($notes) ?></span><?php endif; ?>
      </div>
      <?php endif; ?>
