...
<div class="container py-4" style="max-width:780px;">
  ...
  <?php foreach ($orders as $o):
    ...
    // FIX: shipping info is flat columns
    ...
    <div class="order-card-header">
      <div>
        <div style="font-weight:700;color:var(--deep);font-size:.9rem;">
          <i class="fas fa-tag me-1" style="color:var(--mid);"></i>#<?= htmlspecialchars($orderId) ?>
        </div>
        <?php if ($oDate): ?>
        <div style="font-size:.74rem;color:#999;margin-top:2px;">
          <i class="fas fa-clock me-1"></i><?= date('F d, Y — h:i A', strtotime($oDate)) ?>
        </div>
        <?php endif; ?>
      </div>
      <span class="status-pill <?= $stClass ?> dyn-status-badge">
        <i class="fas fa-circle" style="font-size:.4rem;"></i><?= htmlspecialchars($oStatus) ?>
      </span>
      <?php if ($payMethod): ?>
      <span class="delivery-chip ms-auto">
        <i class="fas fa-credit-card" style="color:var(--mid);"></i><?= htmlspecialchars($payMethod) ?>
      </span>
      <?php endif; ?>
    ...
    <div class="order-card-body">
      ...
      <div style="border:2px solid var(--sage);border-radius:14px;padding:8px 16px;margin-bottom:16px;">
        <?php foreach ($oItems as $oi):
          // FIX: order_items uses product_name column
      ...
      <div class="mt-3 d-flex flex-wrap gap-2">
        ...
        <?php if ($phone): ?>
        <span class="delivery-chip"><i class="fas fa-phone" style="color:var(--mid);"></i><?= htmlspecialchars($phone) ?></span>
        <?php endif; ?>
      </div>
      <?php endif; ?>
