...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['place_order'])) {
    ...
    if (empty($errors)) {
        try {
            ...
            $oStmt = $db->prepare("
                INSERT INTO orders (email, full_name, phone, address, city, province, zip, pay_method, notes, total, created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW())
            ");
            $oStmt->execute([$userEmail, $fullName, $phone, $address, $city, $province, $zip, $payMethod, $notes, $total]);
            $orderDbId = $db->lastInsertId();

            // 2. Insert items into order_items table
            $oiStmt = $db->prepare("
                INSERT INTO order_items (order_db_id, product_id, name, price, qty, image)
                VALUES (?, ?, ?, ?, ?, ?)
            ");
            foreach ($cart as $ci) {
                $pid   = (int)($ci['id'] ?? 0);
                $pname = trim($ci['name'] ?? '');
                $pprice= (float)($ci['price'] ?? 0);
                $pqty  = (int)($ci['qty'] ?? 1);
                $pimg  = trim($ci['image'] ?? '');

                $oiStmt->execute([$orderDbId, $pid, $pname, $pprice, $pqty, $pimg]);
            }
