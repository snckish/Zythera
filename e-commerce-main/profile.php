...
// Build a writable $user array from DB row
$user = [
    'name'        => $dbUser->name ?? '',
    'email'       => $dbUser->email ?? '',
...
if (!isset($_SESSION['profile_pic'][$userEmail])) {
    $_SESSION['profile_pic'][$userEmail] = $user['profile_pic'];
}
    ...
    if (isset($_POST['update_profile'])) {
        ...
        if (!empty($newPass)) {
            ...
            $stmt = $db->prepare("
                ...
                WHERE email = ?
            ");
            $stmt->execute([$newName, $hashed, $userEmail]);
        } else {
            $stmt = $db->prepare("
                UPDATE users
                SET name = ?
                WHERE email = ?
            ");
            $stmt->execute([$newName, $userEmail]);
        }
...
// Load only this user's orders from DB
...
<nav class="navbar navbar-light px-4 py-2 fixed-top">
    ...
    <div class="ms-auto d-flex gap-2 align-items-center">
        ...
        <a href="orders.php" class="btn btn-sm btn-outline-secondary rounded-pill">
           My Orders
        </a>
        <?php else: ?>
    ...
    <div class="section-card">
        ...
        <?php else: ?>
            <?php foreach (array_reverse($orders) as $o): ?>
                ...
                $shippingInfo = !empty($o->shipping_info) ? json_decode($o->shipping_info, true) : [];
                $oShipAddr = $shippingInfo ? implode(', ', array_filter([
